...
void performOperationBasedOnCommand(const String& command)
{
 ...
 // the name below can be changed with your own name and string if you have successfully register your name and face recognition string
  }
  else if (command.startsWith("Axel"))
  {
    // Turn on LED1 and LED2 
    controlLed(led1Pin, HIGH);
    controlLed(led2Pin, HIGH);
    Serial.println("LED1 and LED2 turned on for Axel");
  }
  else if (command.startsWith("Scotley"))
  {
    // Turn on LED2 and LED3 
    controlLed(led2Pin, HIGH);
    controlLed(led3Pin, HIGH);
    Serial.println("LED2 and LED3 turned on for Scotley");
  }
  else if (command.startsWith("Riordan"))
  {
    // Turn on LED1 and LED3 
    controlLed(led1Pin, HIGH);
    controlLed(led3Pin, HIGH);
    Serial.println("LED1 and LED3 turned on for Riordan");
  }
  ...
  if (Firebase.ready() && (millis() - sendDataPrevMillis > 15000 || sendDataPrevMillis == 0))
  {
    ...
    if (Firebase.RTDB.getString(&fbdo, "/commands"))
    {
      if (fbdo.dataType() == "string")
      {
        ...
        performOperationBasedOnCommand(command);
      }
    }
    else
    {
